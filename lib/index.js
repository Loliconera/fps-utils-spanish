	...
	function GuiHandler(page, arg) {
		switch (page) {
			...
			case "help":
				gui.parse([
					{ text: `<font color="${PrpC}" size="+24"><p align="right">MENÚ PRINCIPAL</p></font><br>`, command: "fps gui" },
					{ text: `<font size="20"><font color="${LBlC}">Comando</font>            <font color="${SlvC}">Arg(s)</font>                 <font color="${CPkC}">Ejemplo</font><br>` },
			...
			default:
				gui.parse([
					...
					{ text: `<font color="${Cfg.HideMonsterDeathAnimation ? GrnC : RedC}" size="+18">[Ocultar Muerte Ani]</font>`, command: "fps monsterdeathani;fps gui" }, { text: "&nbsp;&nbsp;" },
					{ text: `<font color="${Cfg.ActionScripts ? GrnC : RedC}" size="+18">[Acercamientos-Zoom]</font>`, command: "fps npczoom;fps gui" }, { text: "&nbsp;&nbsp;" },
					...
					{ text: `<font color="${YlwC}" size="+20">Anormal:</font>` }, { text: "&#09;" },
		...
		switch (key) {
			...
			case "hide":
				if (typeof arg === "string" && arg !== null) {
					if (Cfg.PlayersBlacklist.includes(arg)) return Msg(`Player '${arg}' <font color="${RedC}">Ya escondido</font>.`);
					else
						if ((PClasses.includes(arg) && !Cfg.ClassesBlacklist.includes(arg)) || (PRoles.includes(arg) && !Cfg.RolesBlacklist.includes(arg))) {
							...
							Msg(`Class/Role ${arg} <font color="${GrnC}">Oculto</font>.`);
							return;
						} else if (Cfg.ClassesBlacklist.includes(arg) || Cfg.RolesBlacklist.includes(arg)) return Msg(`Class/Role '${arg}' <font color="${RedC}">Ya escondido</font>.`);
					Msg(`Player '${arg}' <font color="${GrnC}">Oculto</font>.`);
			...
			case "show":
				if (typeof arg === "string" && arg !== null) {
					if (Cfg.PlayersBlacklist.includes(arg)) {
						...
						Msg(`Player '${arg}' <font color="${RedC}">Mostrado</font>.`);
						return;
					}
					if ((PClasses.includes(arg) && Cfg.ClassesBlacklist.includes(arg)) || (Cfg.RolesBlacklist.includes(arg) && PRoles.includes(arg))) {
						...
						Msg(`Class '${arg}' <font color="${RedC}">Mostrado</font>.`);
					...
					else if (!Cfg.PlayersBlacklist.includes(arg)) Msg(`Player '${arg}' <font color="${RedC}">Ya mostrado</font>.`);
					else Msg(`<font color="${GryC}">Argumento no válido: '${arg2}'</font>.`);
				}
				break;
			case "party":
			...
			case "pvptraps":
				...
				Msg(`Pvp Las trampas son ${Cfg.PvpTraps ? `<font color="${GrnC}">Mostrados<font color="${PnkC}">(No se ve afectado por ocultar todos los proyectiles)</font></font>` : `<font color="${RedC}">Normal<font color="${PnkC}">(affected by hide all projectiles)</font></font>`}.`);
				break;
			case "list":
			...
			case "npcs": case "npc":
				if (arg === 'hide') {
					if (!arg2 || !arg3) {
						...
						Msg(`Lista negra NPCs ${Cfg.HideBlacklistedNpcs ? `<font color="${GrnC}">Oculto</font>` : `<font color="${RedC}">Mostrado</font>`}.`);
						break;
					}
