...
module.exports = function FpsUtils(mod) {
		...
		switch (page) {
			...
			default:
				gui.parse([
					...
					{ text: `<font color="${Cfg.Hideguildlogos ? GrnC : RedC}" size="+18">[Logos de Guild]</font>`, command: "fps guildlogo;fps gui" }, { text: "&nbsp;&nbsp;" },
					{ text: `<font color="${Cfg.HideFireworks ? GrnC : RedC}" size="+18">[Fuegos Artificiales]</font>`, command: "fps fireworks;fps gui" }, { text: "&nbsp;&nbsp;" },
					{ text: `<font color="${Cfg.HideServantBalloons ? GrnC : RedC}" size="+18">[Notificacion de Mascotas]</font><br><br>`, command: "fps petspopup;fps gui" },
					{ text: `<font color="${YlwC}" size="+20">Uno mismo (propio):</font>` }, { text: "&#09;" },
					{ text: `<font color="${Cfg.HideHpNumbers ? GrnC : RedC}" size="+18">[HP Números]</font>`, command: "fps hpnumbers;fps gui" }, { text: "&nbsp;&nbsp;&nbsp;" },
					{ text: `<font color="${Cfg.HideMpNumbers ? GrnC : RedC}" size="+18">[MP Números]</font>`, command: "fps mpnumbers;fps gui" }, { text: "&nbsp;&nbsp;&nbsp;" },
					...
					{ text: `<font color="${Cfg.RaidAutoChange ? GrnC : RedC}" size="+18">[Raid estado automático]</font>`, command: "fps raid;fps gui" }, { text: "&nbsp;&nbsp;" },
					...
					{ text: `<font color="${Cfg.GuardianAutoChange ? GrnC : RedC}" size="+17">[Guardian estado automático]</font>`, command: "fps guardian;fps gui" }, { text: "&nbsp;&nbsp;" },
					{ text: `<font color="${Cfg.MuteOthersVoice ? GrnC : RedC}" size="+17">[Silenciar voces de otros]</font>`, command: "fps muteothers;fps gui" }, { text: "&nbsp;&nbsp;" },
	...
	function ShowSpecificPlayerByName(name) {
		for (const hUser in HUsers) {
			if (HUsers[hUser].name.toString().toLowerCase() === name.toLowerCase()) {
				...
				mod.toClient('S_SPAWN_USER', mod.majorPatchVersion >= 99 ? 16 : 15, HUsers[hUser]);
				delete HUsers[hUser];
				break;
			}
		}
	...
	mod.hook('S_SPAWN_USER', mod.majorPatchVersion >= 99 ? 16 : 15, LastHook, sSpawnUser)
	mod.hook('S_SPAWN_USER', mod.majorPatchVersion >= 99 ? 16 : 15, LastHookfn, sSpawnUserfn)
	...
	mod.hook('S_SPAWN_DROPITEM', mod.majorPatchVersion >= 99 ? 9 : 8, LastHook, sSpawnDropItem)
